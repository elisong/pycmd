# -*- coding: utf-8 -*-
# Create python virtual environment on MacOS.
# Usage: venv
...
def create_venv(vern, path):
    ...
    pip_vern = subprocess.run(".venv/bin/pip --version | awk '{print $2}'",
                              shell=True,
                              stdout=subprocess.DEVNULL)
...
def switch_mirror():
    ...
    if index > 0:
        subprocess.run(['.venv/bin/pip', 'config', '--site', '-q',
                        'set', 'global.index-url', mirror])
    Console.ok(mirror)
